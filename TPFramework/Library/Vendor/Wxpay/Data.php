...
class WxPayDataBase
{
	...
	**/
	 ...
	 */
	public function MakeSign($api_key)
	{
		//签名步骤一：按字典序排序参数
		ksort($this->values);
...
class WxPayResults extends WxPayDataBase
{
	 ...
	 * 检测签名
	 */
	public function CheckSign($api_key)
	{
		//fix异常
		if(!$this->IsSignSet()){
	 ...
	 */
	public static function InitFromArray($array, $noCheckSign = false, $api_key = '')
	{
		$obj = new self();
		$obj->FromArray($array);
		if($noCheckSign == false){
			$obj->CheckSign($api_key);
		}
        return $obj;
	}
     ...
     * @throws WxPayException
     */
	public static function Init($xml, $api_key)
	{	
		$obj = new self();
		$obj->FromXml($xml);
		//fix bug 2015-06-29
