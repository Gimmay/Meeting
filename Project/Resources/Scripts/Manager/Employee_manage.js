...
var temp = {
	...
	bindEvent            :function(){
		...
		$('.all_role_area a').on('click', function(){
			...
			Common.ajax({
				data:{requestType:'assign_role', rid:id, id:employee_id}, callback:function(data){
		...
		$('.select_role_area a').on('click', function(){
			...
			ManageObject.object.loading.loading(true);
			Common.ajax({
				data:{requestType:'anti_assign_role', rid:id, id:employee_id}, callback:function(data){
					ManageObject.object.loading.complete();
					if(data.status){
		...
		$('#authorize_select a').on('click', function(){
			...
			ManageObject.object.loading.loading(true);
			Common.ajax({
				data:{requestType:'anti_assign_permission', pid:id, id:employee_id, type:type}, callback:function(data){
					ManageObject.object.loading.complete();
					if(data.status){
...
$(function(){
	...
	$('.password_btn').on('click', function(){
		if($('#old_password').val() == ''){
			ManageObject.object.toast.toast('旧密码不能为空！');
			$('#old_password').focus();
			return false;
		}
		if($('#new_password').val() == ''){
			ManageObject.object.toast.toast('新密码不能为空！');
			$('#new_password').focus();
			return false;
		}
		if($('#new_password2').val() == ''){
			ManageObject.object.toast.toast('确认密码不能为空！');
			$('#new_password2').focus();
			return false;
		}
		if($('#old_password').val() == $('#new_password').val()){
			ManageObject.object.toast.toast('旧密码和新密码不能相同！');
			$('#old_password').focus();
			return false;
		}
		if($('#new_password').val() != $('#new_password2').val()){
			ManageObject.object.toast.toast('两次输入密码不一致，请重新输入！');
			$('#new_password').focus();
			return false;
		}
		return true;
