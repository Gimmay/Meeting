<?php
	...
	interface OfficialAccountNativeAPI{
		 ...
		 * @param int    $mode    模式 可选值包括0或1<br>
		 *                        0：不弹出授权页面，直接跳转，只能获取用户openid<br><br>
		 *                        1：弹出授权页面，可获取用户相信信息，并且即使在未关注的情况下，只要用户授权，也能获取其信息<br>
		 ...
		 * @return array 返回调用结果和openid对应的用户信息
		 */
		public function getUserInformation($openid, $lang = 'zh-CN', $access_token = null);
	...
	class OfficialAccountLibrary implements OfficialAccountNativeAPI{
		...
		public function getAccessToken($cache_path = null){
			$cache_obj = new OfficialAccountCache($this->_config['appID'], 'at', $cache_path);
		 ...
		 */
		public function getUserFullSet($mode = 0, $opt_str = '', $app_id = null, $app_secret = null){
			if(isset($_GET['code'])){
				$code     = $_GET['code'];
		...
		public function getJSAPITicket($cache_path = null, $access_token = null){
			$cache_obj    = new OfficialAccountCache($this->_config['appID'], 'jt', $cache_path);
			$access_token = $access_token == null ? $this->getAccessToken() : $access_token;
			if($cache_obj->isExpired()){
