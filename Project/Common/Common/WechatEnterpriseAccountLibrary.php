<?php
	...
	class EnterpriseAccountLibrary implements EnterpriseAccountNativeAPI{
		...
		public function getAccessToken($cache_path = null){
			$cache_obj = new EnterpriseAccountCache($this->_config['corpID'], 'at', $cache_path);
			if($cache_obj->isExpired()){
				...
				if($response['status']){
					$cache_obj->saveCacheData(json_encode([
						...
						'expiredTime' => time()+$cache_obj::_CONFIG_EXPIRED_TIME
					]));
		...
		public function getCode($url, $data = '', $corp_id = null){
			...
			header("Location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=$corp_id&redirect_uri=$url&response_type=code&scope=snsapi_base&state=$data#wechat_redirect");
		}
		 ...
		 * @param null|string $corp_id 企业号的corpid
		 *
		 * @return null|string 若成功则返回UserID，否则返回null
		 */
		public function getID($opt_str = '', $corp_id = null){
			if(isset($_GET['code'])){
				$code     = $_GET['code'];
				...
				$this->getCode($redirect_url, $opt_str, $corp_id);
				exit;
			}
		}
		...
		public function getJSAPITicket($cache_path = null, $access_token = null){
			$cache_obj    = new EnterpriseAccountCache($this->_config['corpID'], 'jt', $cache_path);
			$access_token = $access_token == null ? $this->getAccessToken() : $access_token;
			if($cache_obj->isExpired()){
	...
	class EnterpriseAccountCache{
		...
		const _CONFIG_EXPIRED_TIME = 7000; // 缓存文件有效时间
		private $_config = [
