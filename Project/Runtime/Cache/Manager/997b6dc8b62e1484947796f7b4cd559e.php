...
<head>
	...
	<style>
		...
		@-ms-keyframes rotate-y{
			0%{ -ms-transform : rotateY(90deg) rotateX(-90deg) }
			15%{ -ms-transform : rotateY(15deg) rotateX(-15deg) }
			85%{ -ms-transform : rotateY(-15deg) rotateX(15deg) }
			100%{ -ms-transform : rotateY(-90deg) rotateX(90deg) }
		}

		.rotateY{
			animation         : rotate-y <?php echo ($waitSecond); ?>s ease both;
			-moz-animation    : rotate-y <?php echo ($waitSecond); ?>s ease both;
			-webkit-animation : rotate-y <?php echo ($waitSecond); ?>s ease both;
			-o-animation      : rotate-y <?php echo ($waitSecond); ?>s ease both;
		}
		...
		.moveY{
			animation         : move-y <?php echo ($waitSecond); ?>s ease both;
			-moz-animation    : move-y <?php echo ($waitSecond); ?>s ease both;
			-webkit-animation : move-y <?php echo ($waitSecond); ?>s ease both;
			-o-animation      : move-y <?php echo ($waitSecond); ?>s ease both;
		}
...
<body>
	<div id="dialog">
		...
		<div id="jump">
			<span id="wait"><?php echo ($waitSecond); ?></span>秒后会自动跳转<br> 若页面无响应请点击<a href="<?php echo ($jumpUrl); ?>">此处</a>
		</div>
		<div class="clean_float"></div>
	</div>
	<div id="foot" class="moveY">
		Copyright ©2016-2017 Quasar.CEAT All Rights Reserved<br> Email:
