...
<div class="mt_topbar clearfix">
					...
					<section class="content">
						<header class="c_header">
							<div class="function_list clearfix">
								<?php if($permission['create'] == 1): ?><div class="function_btn bg-warning" data-toggle="modal" data-target="#create_employee">
										<a href="<?php echo U('Employee/create');?>"> <i></i>
											<p>新增员工</p>
										</a>
									</div><?php endif; ?>
								<?php if($permission['delete'] == 1): ?><div class="function_btn bg-danger batch_delete_btn_confirm" data-toggle="modal" data-target="#batch_delete_employee">
										<i></i>
										<p>批量删除</p>
									</div><?php endif; ?>
							</div>
									...
									<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$single): $mod = ($i % 2 );++$i;?><tr>
											...
											<td>
												<div class="btn-group" data-id="<?php echo ($single["id"]); ?>">
													<?php if($max_role_level <= $single['roleLevel']): if($permission['assignRole'] == 1): ?><button type="button" class="btn btn-default btn-xs btn_distribution_role" data-toggle="modal" data-target="#distribution_role">分配角色</button><?php endif; endif; ?>
													<?php if($permission['assignPermission'] == 1): ?><button type="button" class="btn btn-default btn-xs authorize_btn" data-toggle="modal" data-target="#authorize_role">授权</button><?php endif; ?>
													<?php if($permission['alter'] == 1): ?><a href="<?php echo U('Employee/alter',['id'=>$single['id']]);?>" type="button" class="btn btn-default btn-xs modify_btn">修改</a><?php endif; ?>
													...
													<?php if($permission['delete'] == 1): ?><button type="button" class="btn btn-default btn-xs delete_btn" data-toggle="modal" data-target="#delete_employee">删除</button><?php endif; ?>
												</div>
											</td>
	...
	<!-- 删除员工 -->
	<div class="modal fade" id="delete_employee" tabindex="3" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<!-- 批量删除 -->
	<div class="modal fade" id="batch_delete_employee" tabindex="3" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">
	...
	<div class="modal fade" id="alter_password" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form class="form-horizontal" role="form" action="<?php echo U('alterPassword');?>" method="post">
					<input type="hidden" name="requestType" value="alterPassword">
					<input type="hidden" name="id">
					<div class="modal-body">
						<div class="form-group">
							<label for="old_password" class="col-sm-2 control-label">旧密码：</label>
							<div class="col-sm-10">
								<input type="text" class="form-control old_password" name="old_password" id="old_password">
							</div>
						</div>
						<div class="form-group">
