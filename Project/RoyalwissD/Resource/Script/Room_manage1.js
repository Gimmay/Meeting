		...
		callback:function(r){
			...
			$('.c_add').on('click', function(){
				...
				Common.ajax({
					...
					callback:function(r){
						if(r.status){
							ManageObject.object.toast.toast('换房成功');
							$('#change_room').modal('hide');
							location.reload();
						}else{
							ManageObject.object.toast.toast('换房失败');
						}
					}
				});
			});
...
$(function(){
	...
	$('.alter_btn').on('click', function(){
		...
		choose($(this));
		Common.ajax({
			data    :{requestType:'get_room_type_list', id:id},
			callback:function(r){
				if(r.status){
	...
	$('#distribution_room').find('#room_type').on('change', function(){
		...
		Common.ajax({
			...
			callback:function(r){
				...
				$this_modal.find('#surplus').val(r.number-r.capacity);
			}
		});
	});
	//选择房间类型自动获取数据
...
function checkAssign(){
	...
	if(surplus == 0){
		ManageObject.object.toast.toast('该房间类型房间数不足！');
		return false;
	}
	return true;
}
