try{
	...
	(function($){
		 ...
		 * @constructor
		 */
		$.fn.IcemanAlterPopup = function(options){
			var $ths           = $(this);
			var defaults       = {
					...
					$.ajax({
						...
						success :function(res){
							if(res.status){
								...
								$obj.on('qy-success', opts.success(res));
							}else{
								$obj.trigger('qy-error');
								$obj.on('qy-error', opts.error(res));
							}
						}
					});
				});
				$('.sub_val').on('keydown', function(e){
					if(e.keyCode == 13){
						e.preventDefault();
						$('#alter-popup').find('.alter-submit').trigger('click');
					}
				});
			};
			/**
			...
			var _makeComponent = function(html, val, cid, eve, success, error){
				...
				$ths.parents('table').find('td').css('border', '1px solid #ddd');
				$ths.css('border', '2px dashed #3EB5EB');
				_unbindEvent();
				_bindEvent();
			};
