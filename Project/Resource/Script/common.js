...
$(function(){
	...
	// 判断是否存在 mt_wrapper 类
	$('.nav-tip').off('click').on('click', function(){
		if($(this).hasClass('nav_open')){
			$('.mt_navbar').animate({left:-180+'px'});
			$('.mt_content').animate({paddingLeft:0});
			$('.mt_topbar .header').addClass('other_style');
		}else{
			$('.mt_navbar').animate({left:0});
			$('.mt_content').animate({paddingLeft:180+'px'});
			$('.mt_topbar .header').removeClass('other_style');
		}
	});
});
var Common = {
