try{
	...
	(function($){
		 ...
		 * 参数说明。
		 * 1、组件参数
		 *    classStyle String  应用到每个按钮组的 class 属性中，默认值 'btn btn-default'。
		 *    data Array|String  （一维 JSON 数组或标准的 JSON 字符串）指定按钮的携带数据，数据必须保证以标准的 JSON 对象数组或 JSON 字符串格式传递，每一组数据必须是 {value: val1, html: val2 [, current: val3, classStyle: val4]} 的格式（每组数据中的参数详见第2条说明），默认值为空数组 []。
		 *    defaultValue String  以默认值形式传递到该组件的表单元素，默认值为空字符串 ''。
		 *    multiple boolean  指定是否开启复选模式，默认值为 false。
		 *    separator String  指定该组件在复选模式下的表单元素的值的分隔字符，默认值为 ','。
		 *    canCancel boolean  指定该组件在单选模式下是否取消已选项的状态，默认值为 false。
		 *    name String  指定该组件的表单 name，默认值为 'quasar_button_group'。
		 * 2、数据参数
		 *    value String  指定该按钮的值。
		 *    html String  指定该按钮显示的文本。
		 *    current boolean  会指定是否默认为选择项。
		 *    classStyle String  会在单独指定对应按钮的类。
		 *
		 * @created 2016-09-18 18:19
		 * @updated 2016-09-18 21:32
		 * @author Quasar
		 * @version 1.02
		...
		$.fn.QuasarButtonGroup = function(options){
			...
			var _makeComponent = function(){
				...
				for(var key in opts.data){
					...
					//noinspection JSUnfilteredForInLoop
					if(opts.data[key].hasOwnProperty('value') && opts.data[key].value == opts.defaultValue) active = true;
